...
<div class="container-fluid" id="container-wrapper">
	<div class="d-sm-flex align-items-center justify-content-between mb-4">
		<h1 class="h3 mb-0 text-gray-800">SKP P3</h1>
		<ol class="breadcrumb">
			<li class="breadcrumb-item"><a href="./">Home</a></li>
			<li class="breadcrumb-item active" aria-current="page">SKP P3</li>
		</ol>
	</div>
	<div class="col-lg-12">
		<div class="card mb-4">
			...
			<div class="card-header py-3 d-flex flex-row align-items-center justify-content-between" style="background-color:#e6ffff">
				<h6 class="m-0 font-weight-bold text-primary">Tabel SKP P3</h6>
				<div class="btn-group">
					<!--<a href="<?= base_url("skpp3/tambah") ?>" class="m-0 float-right btn btn-primary btn">
			...
			<div class="table-responsive p-3">
				...
				&nbsp;&nbsp;&nbsp;<a href="<?= base_url("skpp3/filter/Perlengkapan_Berkas_&_Generate_NP2") ?>" class="m-0 float-right btn btn-primary btn-sm">Tunggakan NP2</a>&nbsp;&nbsp;&nbsp;
				<a href="<?= base_url("skpp3/filter/SP2_terbit") ?>" class="m-0 float-right btn btn-warning btn-sm">Tunggakan SP2</a>&nbsp;&nbsp;&nbsp;
				<a href="<?= base_url("skpp3/filter/LHP_terbit") ?>" class="m-0 float-right btn btn-success btn-sm">LHP Terbit</a>&nbsp;&nbsp;&nbsp;
			
				
					<div class="btn-group-mb-1">
						<a href="<?= base_url('skpp3') ?>"><i class="fa fa-history"></i></a>
					</div>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
				...
				<table class="table align-items-center table-flush table-hover" id="dataTableHover">
					...
					<tbody>
						<?php $no=1; foreach($listskpp3 as $value): ?>
						<tr>
						<th style="font-size: 12px; color:#840bb8"><?= $no++ ?></td>
							...
							<td style="font-size: 12px; color: #a70c71"><b><?= $value->t_jenisskp ?></b></td>
							<td style="font-size: 11px"><?php echo number_format($value->t_skpkbterbit, 0, ',', '.' ); ?></td>
							<td style="font-size: 12px"><?= $value->t_skpkbdisetujui ?></td>
							<td style="font-size: 12px"><?= $value->t_skpkbcair ?></td>
							<td style="font-size: 12px"><?= $value->t_sptlb ?></td>
							<td style="font-size: 12px"><?= $value->t_skplbterbit ?></td>
							<td style="font-size: 12px"><?= $value->t_skplbdisetujui ?></td>
							<td style="font-size: 12px"><?= $value->t_rd ?></td>
							<td style="font-size: 12px"><?= $value->t_status ?></td>
							<td style="font-size: 12px; color:#800060"><?= $value->t_keterangan ?></td>
							<td style="font-size: 12px"><?= $value->t_tanggalupdateskpp3 ?></td>
							<td>
								<div class="btn-group dropleft">
									...
									<div class="dropdown-menu">
										<a type="button" href="<?= base_url('skpp3/edit/' . $value->id_datapemrikp3)?>"
											class="dropdown-item"><i class="fa fa-edit"></i> Edit </a>
										<div class="dropdown-divider"></div>
										<a type="button"
											onclick="deleteskpp3('<?=$value->id_datapemrikp3?>','<?=$value->t_namawp?>');"
...
<?php $this->load->view('skpp3/v_modal')?>

<!--Javascript-->

<script type="text/javascript">
	function deleteskpp3(id_datapemrikp3, t_namawp) {
		...
		swal({
			...
			if (submit) {
				$.ajax({
					url: "<?= base_url('skpp3/delete/') ?>" + id_datapemrikp3,
					type: 'delete',
	...
	function filterData() {
		...
		window.location.href = "<?= base_url('skpp3/filter/') ?>" + status ;
	...
	//Export PDF
	function exportskpp3() {
		var id_datapemrikp3 = $('#id_datapemrikp3').val();
		window.open("<?= base_url('skpp3/exportpdf/'. $value->id_datapemrikp3)?>");
