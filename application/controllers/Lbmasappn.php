...
class lbmasappn extends MY_Controller 
{
...
//Index
	public function index()
	{
        ...
        $data['listlbmasappn'] = $this->m_lbmasappn->listlbmasappn();

		$this->template->load('template/v_layout','lbmasappn/v_index', $data);
	}
...
//Filter
	...
	{
        $data['userlogin'] = $this->userlogin;
        $data['listlbmasappn'] = $this->m_lbmasappn->filterlistlbmasappn($status, $tgl_awal, $tgl_akhir);
...
//Tambah
    public function tambah()
	{
		...
		$this->template->load('template/v_layout','lbmasappn/v_tambah', $data);
	}
...
//Simpan
    public function simpan()
	{
        ...
        if($this->m_lbmasappn->insert($in_data))
        {
...
//Edit
    public function edit($id_lbmasappn)
	{
        $data['userlogin'] = $this->userlogin;

        $data['datalbmasappn'] = $this->m_lbmasappn->listlbmasappn_byid($id_lbmasappn);

		$this->template->load('template/v_layout','lbmasappn/v_ubah', $data);
    }

//Update
    public function update($id_lbmasappn)
	{
        $id_data['id_lbmasappn'] = $id_lbmasappn;
        $in_data['t_namawp'] = $this->db->escape_str($this->input->post('namawp'));
        $in_data['t_npwp'] = $this->db->escape_str($this->input->post('npwp'));
        $in_data['t_alamatsurat'] = $this->db->escape_str($this->input->post('alamatsurat'));
        $in_data['t_masa_tahun_pajak'] = $this->db->escape_str($this->input->post('masa_tahun_pajak'));
        ...
        if($this->m_lbmasappn->update($in_data, $id_data))
        {
            $output ['status_code'] = 200; //200, 400, 404, 500
            $output ['title'] = "Berhasil";
...
//Delete
    public function delete($id_lbmasappn)
	{

        if($this->m_lbmasappn->delete($id_lbmasappn))
        {
            $output ['status_code'] = 200; //200, 400, 404, 500
            $output ['title'] = "Berhasil";
...
//Export Excel
    public function exportlistlbmasappn()
	{
        $listdata = $this->m_lbmasappn->exportlistlbmasappn();
        $data = array(
            'title' => 'List Data Usulan Pemeriksaan - ' . time(),
            'listdata' => $listdata
        );
        $this->load->view('lbmasappn/v_listlbmasappn_xls', $data);


	}

//Export PDF
    public function exportpdf($id_lbmasappn)
	{
        $data['userlogin'] = $this->userlogin;
        $id_data['id_lbmasappn'] = $id_lbmasappn;
        $data['datalbmasappn'] = $this->m_lbmasappn->listlbmasappn_byid($id_lbmasappn);
        ...
        $this->pdf->load_view('lbmasappn/v_cetaklbmasappn', $data);
	}
...
//UploadFiles (masih gagal)
    public function uploadfiles() 
    {
        ...
        $this->load->view('template/v_layout','lbmasappn/v_index', $data);

    }

//Import Dari Excel
    public function import_lbmasappn()
    {
        if(isset($_FILES['file']['name']))
        {
            ...
            if($this->m_lbmasappn->import_data($data))
            {
