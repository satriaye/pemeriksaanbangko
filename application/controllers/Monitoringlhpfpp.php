...
class monitoringlhpfpp extends MY_Controller 
{
...
//Login
    ...
    {
        parent::__construct();
        $this->load->model('m_monitoringlhpfpp');
...
//Index
	public function index()
	{
        ...
        $data['listmonitoringlhpfpp'] = $this->m_monitoringlhpfpp->listmonitoringlhpfpp();

		$this->template->load('template/v_layout','monitoringlhpfpp/v_index', $data);
	}
...
//Filter
	...
	{
        $data['userlogin'] = $this->userlogin;
        $data['listmonitoringlhpfpp'] = $this->m_monitoringlhpfpp->filterlistmonitoringlhpfpp($status);
...
//Tambah
    public function tambah()
	{
		...
		$this->template->load('template/v_layout','monitoringlhpfpp/v_tambah', $data);
	}
...
//Simpan
    public function simpan()
	{
        ...
        $in_data['t_tanggallhp'] = $this->db->escape_str($this->input->post('tanggallhp'));
        $in_data['t_tanggalsampaip3'] = $this->db->escape_str($this->input->post('tanggalsampaip3'));
        $in_data['t_tanggalndkepelayanan'] = $this->db->escape_str($this->input->post('tanggalndkepelayanan'));
        ...
        if($this->m_monitoringlhpfpp->insert($in_data))
        {
...
//Edit
    public function edit($id_datapemrikfpp)
	{
        ...
        $data['datadatapemrikfpp'] = $this->m_monitoringlhpfpp->listmonitoringlhpfpp_byid($id_datapemrikfpp);

		$this->template->load('template/v_layout','monitoringlhpfpp/v_ubah', $data);
    }
...
//Update
    public function update($id_datapemrikfpp)
	{
        ...
        $in_data['t_tanggalsampaip3'] = $this->db->escape_str($this->input->post('tanggalsampaip3'));
        $in_data['t_tanggalndkepelayanan'] = $this->db->escape_str($this->input->post('tanggalndkepelayanan'));
        $in_data['t_lhpkonversi'] = $this->db->escape_str($this->input->post('lhpkonversi'));
        $in_data['t_keteranganwaktulhp'] = $this->db->escape_str($this->input->post('keteranganwaktulhp'));
        $in_data['t_status'] = $this->db->escape_str($this->input->post('status'));
        $in_data['t_keterangan'] = $this->db->escape_str($this->input->post('keterangan'));	
        $in_data['t_tanggalupdatemonitoringlhp'] = $this->db->escape_str($this->input->post('tanggalupdatemonitoringlhp'));

        if($this->m_monitoringlhpfpp->update($in_data, $id_data))
        {
            $output ['status_code'] = 200; //200, 400, 404, 500
            $output ['title'] = "Berhasil";
...
//Delete
    public function delete($id_datapemrikfpp)
	{
        ...
        if($this->m_monitoringlhpfpp->delete($id_datapemrikfpp))
        {
            $output ['status_code'] = 200; //200, 400, 404, 500
            $output ['title'] = "Berhasil";
...
//Export Excel
    public function exportlistmonitoringlhpfpp()
	{
        $listdata = $this->m_monitoringlhpfpp->exportlistmonitoringlhpfpp();
        $data = array(
            'title' => 'List Data Usulan Pemeriksaan - ' . time(),
            'listdata' => $listdata
        );
        $this->load->view('monitoringlhpfpp/v_listmonitoringlhpfpp_xls', $data);
...
//Export PDF
    public function exportpdf($id_datapemrikfpp)
	{
        $data['userlogin'] = $this->userlogin;
        $id_data['id_datapemrikfpp'] = $id_datapemrikfpp;
        $data['datamonitoringlhpfpp'] = $this->m_monitoringlhpfpp->listmonitoringlhpfpp_byid($id_datapemrikfpp);
        
        $this->pdf->setPaper('A4','portrait');
        $this->pdf->filename = "Cetak ND & S.pdf";
        $this->pdf->load_view('monitoringlhpfpp/v_cetakmonitoringlhpfpp', $data);
	}
...
//UploadFiles (masih gagal)+
    public function uploadfiles() 
    {
        ...
        $this->load->view('template/v_layout','monitoringlhpfpp/v_index', $data);

    }

//Import Dari Excel
    public function import_monitoringlhpfpp()
    {
        if(isset($_FILES['file']['name']))
        {
            ...
            if($this->m_monitoringlhpfpp->import_data($data))
            {
