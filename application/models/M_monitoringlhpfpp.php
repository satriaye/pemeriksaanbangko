...
class M_monitoringlhpfpp extends CI_Model
{

    private $tbl_datapemrikfpp = 'tbl_datapemrikfpp';
   
    public function listmonitoringlhpfpp()
    {
   ...
   // }
    ...
    public function listmonitoringlhpfpp_byid($id_datapemrikfpp)
    {
        return $this->db->get_where($this->tbl_datapemrikfpp, ['id_datapemrikfpp' => $id_datapemrikfpp])->row();
    }

    //Filter ListPeminjaman
    public function filterlistmonitoringlhpfpp($status)
    {
        if ($status == "semua")
        $filter = "";
        else 
